...
const PlaylistPage = () => {
	...
	const handleAddSong = async (song) => {
		if (!token) {
			toast({
				title: "Помилка",
				description: "Увійдіть, щоб додавати пісні до плейлиста",
				status: "error",
				duration: 3000,
				isClosable: true,
			});
		...
		setAddingSong(true);
		try {
			const songIds = [...data.songs.map(s => s._id), song._id];
			await client.patch(`/playlists/${id}`, 
				{ 
					title: data.title,
			...
			toast({
				title: "Помилка",
				description: "Не вдалося додати пісню до плейлиста",
				status: "error",
				duration: 3000,
				isClosable: true,
			});
